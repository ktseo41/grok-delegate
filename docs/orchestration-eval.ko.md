...
| fable + deepseek 워커 | 71/72 (98.6%) — 47분(최장); 워커 12/12 완주(재시도 3회는 전부 출력 형식 문제, 수집 실패 0); 유일한 손실은 Fed 결정일에 발효일을 적은 1셀 | 24/72 — 워커 8/12이 무응답(채널 결함, 라운드 1 재측정 전 상태); 완주한 셀은 전부 정답(24/24) |
...
sonnet 워커의 잘못된 지수 선택이 결과에 그대로 실렸다. deepseek 워커는 실행 채널(codex
CLI 경유)의 결함이 변수였다: 채널 결함이 있던 라운드 2에서는 워커 8/12이 무응답으로
대부분을 비웠지만, 채널을 수리해 돌린 라운드 1에서는 12/12 완주에 71/72 — 완주만 하면
정확도는 상위권과 같은 급이다 (두 라운드 완주분 95/96).
...
| fable + deepseek 워커 | fable-5 | 3,176 | 18,750 | 68,874 | 760,200 | — |
| | deepseek-v4-flash (세션 15개) | 18,661,070¹ | 187,377² | — | — | — |
...
¹ 캐시된 입력 11,094,784 포함 — deepseek 계량기는 캐시 적중분을 입력 합계 *안에* 포함해
보고하므로(Claude처럼 별도 cache 열로 계측하지 않음), cache 열이 아니라 각주로 둔다.
입력이 유난히 큰 것은 deepseek 워커의 수집 경로가 셸 fetch(페이지 원문을 통째로 읽음)이기
때문이다. ² 추론 토큰 72,599 포함. ³ grok CLI는 청구용 in/out을 공개하지 않고 세션의 최종 컨텍스트
...
7. **워커의 완주율은 채널에서 고치고, 워커 모델은 완주분 정확도로 비교한다.** 워커가
   ...
   직렬화된다 —, 수집 게이트, 재시도 예산 확대, 최종 메시지 형식 규칙) 라운드 1 완주율이
   12/12이 됐다. 워커 *모델*끼리 비교하는 지표는 **완주분 정확도**다: grok 144/144,
   deepseek 95/96 (유일 손실은 Fed 결정일에 발효일을 적은 1셀), sonnet 141/144 (손실 3셀
   전부가 일본 지수 함정 1건의 연쇄). 점수만 보면 완주 실패와 판단 오류가 구분되지
   않으므로, 실패율을 점수 옆에 함께 보고해야 한다(위 요약 표처럼).
...
- **점수는 2026-07-11 통일 재채점 기준이다.** 본 실험 당시에는 실행마다 채점 방식이
  ...
  워크스페이스에), 통일 재채점에서는 독립 세션 세 개가 서로 다른 셔플로 같은 리포트들을
...
- **후속 실험 후보.** ① fable + sonnet 워커 + 재검증 패스를 판단 함정형 과제로 —
  ...
  보탠다. ③ 라운드 2를 수리된 deepseek 채널로 재측정 — 라운드 1에서 채널 수리(최소
  `CODEX_HOME` 내장, 수집 게이트, 재시도 예산 확대)로 완주율이 8/12 무응답에서 12/12이
  됐으니, 판단 함정형에서도 같은 회복이 나오는지 확인할 가치가 있다. 단가는 sonnet의
