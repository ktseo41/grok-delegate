...
모든 점수는 하나의 채점 절차로 매긴 것이다.
라운드마다 블라인드로 채점했다. 블라인드 채점이란 리포트를 어느 조합인지 모르게 한 채로 채점한 것이다.

<sub>*오케스트레이터(fable)가 워커의 결과를 하나하나 재검증 및 교정하는 변형 workflow 실험도
했지만, workflow를 테스트하는 목적이 아니라 grok 위임이 정말 효과적인지 보는 것이 목적이므로
제외했다. 결과만 요약하면: grok 워커 기준 점수는 위 표와 같은 만점이었고 Claude 소모만
2–3배($6.09–7.66)였다 — 원자료는 실험 워크스페이스에.*</sub>

만점의 비결은 범위를 좁힌 것이다. 워커 하나가 중앙은행 한 곳만 맡았고, 워커 프롬프트에
"모든 주장은 이 세션에서 실제로 가져온 페이지에서만, 기억 기반 답변은 무효"라는 규칙을
넣었다. 이 두 가지만으로, 아무도 이중 확인해 주지 않는데도 grok 워커는 두 과제 유형 모두
만점을 유지했고 sonnet 워커도 조회형에서는 같은 만점이었다 — 다만 판단 함정형에서는
sonnet 워커의 잘못된 지수 선택이 결과에 그대로 실렸고, deepseek은 두 라운드 모두 채널
실패로 대부분의 주제를 비웠다. deepseek 조합의 문제는 오로지 완주율이다 — 완주한 셀이
틀린 적은 한 번도 없다 (두 라운드 누적 36/36).

sonnet 단독이 "3회 실행"인 이유: 라운드 2의 세 실행 중 두 개에는 advisor(fable)라는 상위
모델 자문 도구를 붙였는데 **한 번도 발동하지 않았다** (fable advisor 기능 자체는 사전
검증했었다. 따라서 오류가 아니라 모델이 부르지 않기로 한 것이고, "필요하면 물어보라"는 한 줄
힌트를 줘도 마찬가지였다). 처치가 전달되지 않았으므로 세 실행은 모두 sonnet 단독의
반복이고, 점수 분포(70/72, 69/72, 69/72)가 실행 간 변동폭 추정치다. 표와 차트에서는
평균(69.3/72)으로 묶었다.
이번 실험이 advisor 기능이 잘 작동하는지에 중점을 맞춘게 아니기 때문에, 추가로 실험을 진행하지는 않고, sonnet 3회 실행의 평균 점수만 기록했다.
...
| 조합 | 모델 | in | out | cache write | cache read | ctxTokens³ |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| fable + grok 워커 | fable-5 | 3,025 | 16,184 | 73,594 | 375,824 | — |
| | grok-4.5 (워커 세션 12개) | — | — | — | — | 735,055 |
...
| grok 단독 | grok-4.5 (세션 1개) | — | — | — | — | 141,786 |
...
| --- | ---: | :-- |
| fable + grok 워커 | $2.69 | grok: SuperGrok 주간 쿼터 ≈5%p (ctxTokens 비례 추정) |
| fable + deepseek 워커 | $5.06 | deepseek ≈ $0.09 (OpenRouter 요금; 크레딧 소진으로 실행이 도중 잘림) |
| fable + sonnet 워커 | $11.05 | — |
| sonnet 단독 | $5.49 | — |
| fable 단독 | $8.84 | — |
| grok 단독 | $0 | grok: SuperGrok 주간 쿼터 ≈1%p (ctxTokens 비례 추정) |
...
| grok 단독 | grok-4.5 (세션 1개) | — | — | — | — | 161,675 |
...
| 조합 | Claude 쪽 (측정값, API 환산) | 외부 소모 |
| --- | ---: | :-- |
| **fable + grok 워커** | **$2.97** | grok: SuperGrok 주간 쿼터 ≈5%p (ctxTokens 비례 추정; 세션 21개, 게이트에 걸린 저렴한 재시도 9개 포함) |
| fable + deepseek 워커 | $6.83 | deepseek ≈ $0.36 (OpenRouter 요금, 캐시 할인 미적용) |
| fable + sonnet 워커 | $11.54 | — |
| sonnet 단독 (3회 평균) | $5.99 | — |
| fable 단독 | $9.95 | — |
| grok 단독 | $0 | grok: SuperGrok 주간 쿼터 ≈1%p (실측) |
