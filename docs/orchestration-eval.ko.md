...
<sub>*요약 차트: 두 라운드를 합산한 조합별 손실 셀(144개 기준)과 API 환산 총비용 —
...
- **후속 실험 후보.** ① fable + sonnet 워커 + 재검증 패스를 판단 함정형 과제로 —
  ...
  못한다. ④ 같은 grok 워커 위에 오케스트레이터만 바꿔보기 — fable 자리에 opus나
  sonnet을 앉히면, 이 승리가 오케스트레이션 구조 덕인지 오케스트레이터 모델 덕인지
  분리된다. sonnet 변형은 통상 구도가 뒤집히는 경우(워커인 grok-4.5가 더 강한 모델)라,
  오케스트레이터가 더 약해도 분할·조립이 성립하는지의 검증이기도 하다.
