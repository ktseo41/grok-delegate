...
| --- | --- | --- |
| **fable + grok 워커** | **72/72 (100%)** — 워커 12/12 1차 완주, 재시도 0회 | **72/72 (100%)** |
...
| fable + deepseek 워커 | 71/72 (98.6%) — 워커 12/12 완주; 유일한 손실은 Fed 결정일에 발효일을 적은 1셀 | 24/72 — 워커 8/12이 무응답(채널 결함, 라운드 1 재측정 전 상태); 완주한 셀은 전부 정답(24/24) |
...
가져온 페이지에서만"을 강제했다. 이 조건에서 워커 조합의 차이는 셋으로 갈렸다 — grok
워커는 두 과제 유형 모두 만점, sonnet 워커는 조회형은 만점이지만 판단 함정형에서 잘못된
지수 선택이 그대로 실렸고, deepseek 워커는 실행 채널만 고치면 완주하고 정확도도
상위권이다 (두 라운드 완주분 95/96).
...
<sub>¹ 캐시된 입력 11,094,784 포함 — deepseek 계량기는 캐시 적중분을 입력 합계 *안에* 포함해 보고하므로(Claude처럼 별도 cache 열로 계측하지 않음) cache 열이 아니라 각주로 둔다. 입력이 유난히 큰 것은 deepseek 워커의 수집 경로가 셸 fetch(페이지 원문을 통째로 읽음)이기 때문.<br>
² 추론 토큰 72,599 포함.<br>
³ grok CLI는 청구용 in/out을 공개하지 않고 세션의 최종 컨텍스트 크기(ctxTokens)만 남긴다 — 별도 계량기의 값이라 이 표의 다른 열과 합산할 수 없다.<br>
⁴ haiku는 실험이 돌린 모델이 아니라 Claude Code의 WebSearch/WebFetch 도구가 가져온 페이지를 소화할 때 내부적으로 쓰는 모델 — 즉 haiku 행은 *Claude 쪽 세션이* 웹을 얼마나 조회했는지의 계측이다. grok·deepseek 워커 조합은 오케스트레이터의 웹 도구를 끈 채 돌아 haiku가 0(행 생략)이고, sonnet 워커 조합의 haiku 행은 *워커들의* 웹 트래픽이다(워커가 같은 Claude 세션 안에서 돈다).</sub>
...
<sub>¹ 캐시된 입력 2,095,104 포함 — deepseek 계량기는 캐시 적중분을 입력 합계 *안에* 포함해 보고하므로 cache 열이 아니라 각주로 둔다.<br>
² 추론 토큰 포함.<br>
³ ⁴ 위 라운드 1 표의 각주와 동일.</sub>
