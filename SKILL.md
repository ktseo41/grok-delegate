---
name: grok-delegate
description: Delegate a coding, review, or research task to the grok CLI (xAI, grok-4.5) from inside Claude Code, as a subagent-like offload — run it headless via Bash (`grok -p`), in read-only or autonomous mode, and in parallel via background Bash for fan-out. Use when the user wants a second opinion from a different model family, wants to offload work so it does not consume the Claude usage quota (grok bills to a separate xAI quota), wants grok to review or fix code, or says things like "delegate to grok", "ask grok", "grok review", "run grok in parallel", "grok subagent", "cross-check with grok". Not for tasks that need this Claude session's own context — grok always starts fresh.
---
